...
// index 0 -> X
// index 1 -> YAW   (use WYAW here, not WY)
// index 2 -> Z
// index 3 -> PITCH
...
// // ============================================================================
// // RBF center generator
// // ============================================================================

// static void decode_center(std::uint64_t idx,
...
//   for (int d = 0; d < 8; ++d) {
//     int digit = static_cast<int>(q % std::uint64_t(ne));
//     q /= std::uint64_t(ne);
//     c_out[d] = lo + digit * step;
//   }
// }
...
public:
  ...
  ~DataProcessorNode() override {
    ...
    if (wp_csv_.is_open()) wp_csv_.close();
  }
...
private:
  ...
  void setup_params_() {
    ...
    // Waypoint CSV params
    wp_log_      = this->declare_parameter<bool>("wp_log", true);
    wp_csv_path_ = this->declare_parameter<std::string>("wp_csv_path", "waypoint.csv");
    wp_decim_    = this->declare_parameter<int>("wp_decim", 1);
  ...
  void setup_io_() {
    ...
    // Visualization publishers for P_world and p_
  ...
  void setup_math_() {
    ...
    H2_[WZ*4 + WZ]         = 300.0;  
    H2_[WPITCH*4 + WPITCH] = 50.0;

    // Initial conditions for the waypoint generator
    
    // circle
    const std::array<double,4> q0  = { 0.0, 10, z_ref, 0.0};
    const std::array<double,4> qd0 = { 0.1, 0.0, 0.0, 0.0 }; 

    // const double A = 10.0; // radius -> x,y in [-50, 50]
    // const double B = 10.0;    // Y amplitude (meters)
    // const std::array<double,4> q0  = { A, 0.0, z_ref, 0.0};
    // const std::array<double,4> qd0 = { A*wx, A*wx, z_amp * wz_traj, pitch_amp * wp_traj};
    

    // // Number "8" trajectory
    // const double A = 10.0;   // X amplitude (meters)
    // const double B = 5.0;    // Y amplitude (meters)
    // const std::array<double,4> q0  = { 0.0, 0.0, z_ref, 0.0 };      // start at origin in XY
    // // const std::array<double,4> qd0 = { A*wx, B*wy, z_amp * wz_traj, pitch_amp * wp_traj};  
    // const std::array<double,4> qd0 = { A*wx, B*wy, z_amp * wz_traj, 0.0};      
  ...
  void odom_callback(const nav_msgs::msg::Odometry::SharedPtr msg)
  {
    ...
    // Lazily create thruster publishers once we know the prefix
    if (!pub_heave_bow_) {
  ...
  void process_data()
  {
    ...
    z1_[WX]     = x_e; //P_world[X] - p_[WX];                   // ex
    z1_[WYAW]   = wrapToPi(P_world[YAW] - psi_d);        // e_yaw
    ...
    z2_[WX]     = v_t - beta_[WX]; //V_world[X]     - beta_[WX];     // eẋ
    z2_[WYAW]   = V_world[YAW]   - beta_[WYAW];   // e_ψ̇ (yaw rate error)
    ...
    if (pub_z1_odom_) {
      ...
      z1_msg.pose.pose.position.y = P_world[Y] - p_[WY]; // unused in control, but used in error visualization
      ...
      z1_msg.twist.twist.linear.y  = 0.0;          // unused, 
    ...
    float x_f[8];
    x_f[0] = static_cast<float>(X_world[X] / 10.0);           // scale_pos_
    x_f[1] = static_cast<float>(X_world[YAW] / 4.0);         // scale_ang_
    x_f[2] = static_cast<float>(X_world[Z]/5.0);           // scale_pos_
    x_f[3] = static_cast<float>(X_world[PITCH] * 5.0);       // scale_ang_
    x_f[4] = static_cast<float>(X_world[X + 6]);       // scale_lin_
    x_f[5] = static_cast<float>(X_world[YAW + 6] *10.0);     // scale_angvel_
  ...
  const double wx         = 0.01;
  const double wy         = 1.0 * wx;   // 2.0 * wx;    
