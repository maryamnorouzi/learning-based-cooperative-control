...
private:
  ...
  void process_data()
  {
    ...
    // Since the offsets are constant, desired velocity is the same as leader velocity
    ...
    tf2::Quaternion q_wb;  // make a quaternion variable called q_wb so we can store and use orientation for math.
    ...
    double e_y = std::abs(P_world[Y] - p_ref_[WY]);
